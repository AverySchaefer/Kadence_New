...
async function getLocalRecommendations(token, prefData, chosenMood, totalSongs) {
    const { access_token: accessToken } = await refreshToken(token);
...
handler.get(async (req, res) => {
    ...
    const response = await getLocalRecommendations(
        accessToken,
        prefData,
        chosenMood,
        playlistLength
    ...
    while (lengthDifference > 0) {
        ...
        const newResponse = await getLocalRecommendations(
            accessToken,
            prefData,
            chosenMood,
            lengthDifference
