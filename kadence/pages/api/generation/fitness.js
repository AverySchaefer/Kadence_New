...
async function generateSearchParams(songSeedID, prefData, heartrate) {
...
async function getFitnessRecommendations(token, prefData, heartrate) {
    const { access_token: accessToken } = await refreshToken(token);
...
handler.get(async (req, res) => {
    ...
    let response = await getFitnessRecommendations(
        accessToken,
        prefData,
        currentHeartRate,
    );
    ...
    while (currentSongItems.item.name === songItems.tracks[0].name) {
        ...
        response = await getFitnessRecommendations(
            accessToken,
            prefData,
            currentHeartRate
        );
