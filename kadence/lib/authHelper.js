...
export default async function loginWithGithub(callback) {
    /**
     * Fetches the CSRF token, needed for all requests
     * Bear in mind we need to set credentials to include
     * https://developer.mozilla.org/en-US/docs/Web/API/Request/credentials
     */
    const { csrfToken } = await fetch(`${appConfig.apiHost}/api/auth/csrf`, {
        credentials: 'include',
    }).then((res) => res.json());
     ...
     */
    const { url } = await fetch(`${appConfig.apiHost}/api/auth/signin/github`, {
        method: 'POST',
        headers: {
